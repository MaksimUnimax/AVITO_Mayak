...
PATTERNS = (
    re.compile(
        r"(?i)(authorization\s*[\"']?\s*[:=]|bearer\s+[A-Za-z0-9._-]+|password\s*[\"']?\s*[:=]|password@|session[_ -]?cookie|private[_ -]?key)"
    ),
    re.compile(r"(?i)(raw_provider|provider_payload|request_body|response_body)"),
)
    ...
    for name in paths:
        ...
        for pattern in PATTERNS:
            if pattern.search(data):
                findings.append({"filename": name, "finding": "unsafe-pattern"})
...
def main() -> None:
    ...
    print(f"RF24_AMBIGUOUS_SCANNER_FINDINGS={result['finding_count']}")
