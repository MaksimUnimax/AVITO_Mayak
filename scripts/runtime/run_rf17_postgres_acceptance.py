...
def produce(dsn: str, fixture_dsn: str) -> dict[str, object]:
    ...
    with fixture.begin() as connection:
        connection.execute(
            text(
                "grant select,insert,update,delete on all tables in schema mayak to mayak_application"
            )
        )
        connection.execute(
            text("grant usage,select,update on all sequences in schema mayak to mayak_application")
        )
    now = datetime.now(UTC)
    fp = "1" * 64
    ...
    with Session(app) as session:
        for eid in endpoint_ids:
            register_endpoint(
                session,
                EndpointEligibility(eid, account, "GENERIC", f"target-{eid.hex[:8]}"),
                now=now,
            )
    before = _foreign_witness(fixture)
    with Session(app) as session:
        first_fanout = fanout_event(session, event.id, endpoint_ids, now=now)
    with Session(app) as session:
        second_fanout = fanout_event(session, event.id, endpoint_ids, now=now)

    def claim_one() -> list[dict[str, str]]:
        with Session(app) as session:
            return [
                {"outbox_id": str(item.outbox_id), "lease_token": str(item.lease_token)}
    ...
    with Session(app) as session:
        history = [
            ...
            else {name: getattr(entry, name) for name in entry.__slots__}
            for entry in read_history(session, account_id=account, beacon_id=beacon)
        ]
    schema = {
    ...
    return {
        ...
        "alembic_head": ScriptDirectory.from_config(Config("alembic.ini")).get_heads()[0],
        "tables": sorted(NOTIFICATION_TABLES),
