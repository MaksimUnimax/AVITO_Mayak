...
def _upgrade(root: Path, dsn: str, revision: str) -> None:
...
def _run_runtime(session: Session) -> dict[str, Any]:
    ...
    row_before = session.execute(select(metadata.tables["mayak.beacon_beacons"]).where(
        metadata.tables["mayak.beacon_beacons"].c.id == beacon
    )).mappings().one()
    bad_transition = False
    try:
        runtime.activate(
            session, actor_reference="owner", beacon_id=beacon,
            idempotency_key="rf13-activate", expected_row_version=row_before["row_version"],
        )
    except RuntimeError:
        bad_transition = True
    ...
    system_event_actor = session.execute(text(
    ...
    return {
        ...
        "beacons": [str(beacon), str(rollback_id), str(retry_id), str(expiry_id)],
...
def _schema_observations(session: Session) -> dict[str, Any]:
    ...
    columns = {
        table: [column["name"] for column in inspector.get_columns(table, schema="mayak")]
        for table in OWNED
    }
...
def _cleanup(engine: Engine, evidence: dict[str, Any]) -> dict[str, int]:
    beacons = evidence["runtime"]["beacons"]
    account_ids = (evidence["runtime"]["owner"], evidence["runtime"]["foreign"])
...
def run(root: Path, dsn: str, output: Path, technical_id: str, candidate_sha: str,
    ...
    engine = create_engine(dsn, future=True)
    _upgrade(root, dsn, "head")
    ladder = {"empty_to_head": {"before": "empty", "after": RF13_HEAD}}
    if prior_dsn:
        _upgrade(root, prior_dsn, RF12_HEAD)
        _upgrade(root, prior_dsn, "head")
        ladder["rf12_to_head"] = {"before": RF12_HEAD, "after": RF13_HEAD}
    ...
    observations: dict[str, Any] = {
        ...
        "credential_scan": {
            ...
            "exposure": any(
                marker in subprocess.run(
                    ("git", "grep", "-I", "-n", marker, "--", "."), cwd=root,
    ...
    observations["gates"] = {
        ...
        "migration_rf12_to_head": ladder.get("rf12_to_head", {}).get("after") == RF13_HEAD,
