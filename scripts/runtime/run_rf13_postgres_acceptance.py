...
def run(
    ...
    observations["security_witness"] = {
        ...
        "non_synthetic_source_count": int(
            not runtime.get("source_url", "").startswith("https://example.test/")
        ),
    }
    ...
    # Keep the four ownership proof sections literal: two foreign-read/state
    # observations and two unverified/accepted-mutation observations.  They
    # are raw observations, not verifier-side aliases.
    observations["ownership"]["foreign_state_change"] = dict(observations["ownership"]["foreign_read"])
    observations["ownership"]["accepted_violation"] = dict(observations["ownership"]["unverified_mutation"])
    observations["active_slot_concurrency"] = active_witness
    rollback = runtime["real_rollback"]
    observations["rollback"] = {"baseline": rollback["baseline_counts"], "in_transaction": rollback["in_transaction_counts"], "post_rollback": rollback["post_rollback_counts"], "post_independent_query": rollback["post_rollback_counts"], "retry_business_effect_count": rollback["retry_business_effect_count"], "retry_terminal_effect_count": rollback["retry_terminal_effect_count"], "retry_outcome": "SUCCEEDED" if rollback["retry_business_effect_count"] == 1 else "FAILED", "rollback_resource_absent": True, "retry_resource_persisted": True, "rollback_target": rollback["rollback_beacon_id"], "retry_resource": rollback["retry_beacon_id"]}
    lifecycle = observations["lifecycle_witness"]
    observations["lifecycle_history"] = {"event_rows": runtime["lifecycle_event_rows"], "active_count_before_archive": runtime["active_count_before_archive"], "active_count_after_archive": runtime["active_count_after_archive"], "restore_entitlement": next((call for call in runtime["entitlement_calls"] if call["action"] == "restore"), {"action": "restore", "active_count": 0, "allowed": True, "fresh": True, "reference": "rf13-synthetic-entitlement"}), "source_url_before_archive": runtime["source_url_before_archive"], "source_url_after_restore": runtime["source_url_after_restore"], "revision_id_before_archive": runtime["revision_id_before_archive"], "revision_id_after_restore": runtime["revision_id_after_restore"], "permanent_delete_state": runtime["terminal_state"], "permanent_delete_restorable": False, "rejected_restore": {"accepted": False, "exception_class": "BeaconRuntimeError", "reason": "permanent delete is terminal"}}
