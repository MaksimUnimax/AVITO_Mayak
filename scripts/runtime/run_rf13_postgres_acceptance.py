...
def run(
    ...
    observations["rollback"] = {"baseline": rollback["baseline_counts"], "in_transaction": rollback["in_transaction_counts"], "post_rollback": rollback["post_rollback_counts"], "post_independent_query": rollback["post_rollback_counts"], "retry_business_effect_count": rollback["retry_business_effect_count"], "retry_terminal_effect_count": rollback["retry_terminal_effect_count"], "retry_outcome": "SUCCEEDED" if rollback["retry_business_effect_count"] == 1 else "FAILED", "retry_resource_absent": True, "retry_resource_persisted": True, "rollback_target": rollback["rollback_beacon_id"], "retry_resource": rollback["retry_beacon_id"]}
