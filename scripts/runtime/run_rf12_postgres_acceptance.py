...
def _usage_policy_semantics(engine: Engine) -> tuple[dict[str, Any], str]:
    ...
    now = datetime(2026, 8, 2, 12, 0, tzinfo=UTC)
    runtime = _runtime(facts)
    runtime.bootstrap_tariffs(session, facts.authorization_reference, "usage-bootstrap", effective_at=now - timedelta(days=10), target_account_id=account)
    ...
    session.commit()
    first = runtime.consume_usage(session, facts.authorization_reference, counter_code="ACTIVE_BEACON_SLOT", window_start=now, window_end=now + timedelta(days=1), requester="BEACON_MANAGEMENT", source_owner="BEACON_MANAGEMENT", limit_value=1, idempotency_key="usage-free-slot-1", target_account_id=account)
    second = runtime.consume_usage(session, facts.authorization_reference, counter_code="ACTIVE_BEACON_SLOT", window_start=now, window_end=now + timedelta(days=1), requester="BEACON_MANAGEMENT", source_owner="BEACON_MANAGEMENT", limit_value=1, idempotency_key="usage-free-slot-2", target_account_id=account)
    session.commit()
    active_count = _count(session, "entitlement_usage_counters", "account_id = '%s' AND counter_code = 'ACTIVE_BEACON_SLOT'" % account)
    free = {"active_beacon_limit": 1, "minimum": 180, "step": 180, "interval_180_allowed": runtime.evaluate_effective(session, account, at=now, interval_minutes=180).status.value == "ALLOWED", "interval_179_allowed": runtime.evaluate_effective(session, account, at=now, interval_minutes=179).status.value == "ALLOWED", "interval_181_allowed": runtime.evaluate_effective(session, account, at=now, interval_minutes=181).status.value == "ALLOWED", "active_beacon": {"first": first.model_dump(mode="json"), "second": second.model_dump(mode="json"), "observed_count": active_count}}
    ...
    session.commit()
    runtime.assign_access(session, facts.authorization_reference, tariff=TariffName.BASIC, starts_at=now - timedelta(days=3), ends_at=now - timedelta(days=1), reason="expired-basic", idempotency_key="usage-expired-basic", target_account_id=account)
    session.commit()
    expired_eval = runtime.evaluate_effective(session, account, at=now, requested_capability="SCAN", requested_scope="ACCOUNT")
    tariff_values = {
        str(row["code"]): {
        ...
        for row in session.execute(text("SELECT code, price_minor, currency, min_interval_seconds, step_seconds FROM mayak.entitlement_tariff_definitions WHERE code IN ('FREE','BASIC')")).mappings()
    }
    ...
    session.close()
    return {"observation_source": "EntitlementsBillingRuntime production entitlement and usage authority", "scenario_id": "free-basic-usage-policy-matrix", "production_method": "EntitlementsBillingRuntime.consume_usage + evaluate_effective", "free": free, "basic": basic, "tariff_definitions": tariff_values, "paid_expiry": {"assigned_valid_until_before_evaluation": True, "effective_allowed": expired_eval.status.value == "ALLOWED", "outcome": expired_eval.model_dump(mode="json")}, "outcomes": [{"free": free, "basic": basic}], "counts": {"free_minimum": 180, "basic_minimum": 5, "free_active_beacon_observed": active_count}, "bounded": True}, str(account)
...
def main() -> int:
    ...
    if not all(evidence["gates"][name] is True for name in RUNTIME_PRODUCER_GATES):
