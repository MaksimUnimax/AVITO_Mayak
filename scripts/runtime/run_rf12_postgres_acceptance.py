...
def _usage_policy_semantics(engine: Engine) -> tuple[dict[str, Any], str]:
    ...
    tariff_values = {str(row.code): {"price_minor": row.price_minor, "currency": row.currency, "period": row.period} for row in session.execute(text("SELECT code, price_minor, currency, period FROM mayak.entitlement_tariff_definitions WHERE code IN ('FREE','BASIC')")).mappings()}
