...
def _parity(engine: Engine) -> dict[str, Any]:
    ...
    for name in OWNED_TABLES:
        ...
        for column in actual_columns:
            ...
            if str(column["type"]).lower() != str(expected_type).lower():
        ...
        actual_indexes = {i["name"] for i in db.get_indexes(name, schema="mayak")}
...
def _real_payment_race(engine: Engine) -> dict[str, Any]:
    ...
    result = (
        ...
        and any("error" in item for item in cross["outcomes"])
...
def _constraints(engine: Engine) -> dict[str, Any]:
    ...
    with engine.begin() as conn:
        ...
        now = datetime.now(UTC)
        values = {"id": uuid4(), "account": account, "tariff": tariff, "kind": "TARIFF", "cap": None, "scope": None, "reason": "valid", "now": now, "end": now + timedelta(days=1), "row_version": 1}
        positive = "INSERT INTO mayak.entitlement_access_grants (id,account_id,tariff_id,source_code,grant_kind,granted_capability,granted_scope,reason,valid_from,valid_until,state,row_version) VALUES (:id,:account,:tariff,'RF12',:kind,:cap,:scope,:reason,:now,:end,'ACTIVE',:row_version)"
...
def produce(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with engine.begin() as conn:
        ...
        head = conn.execute(text("SELECT version_num FROM mayak.alembic_version")).scalar_one()
    ...
    evidence = {
        ...
        "alembic_head": head, "historical_rf12_manual_grant_sha256": _sha(HISTORICAL),
        ...
        "metadata_parity": _parity(engine), "constraint_matrix": _constraints(engine), "command_matrix": matrix,
    ...
    evidence["gates"]["command_matrix"] = {row["command_id"] for row in matrix["rows"] if "command_id" in row} == set(COMMAND_IDS)
...
def main() -> int:
    ...
    try:
        ...
        print(f"RF12 acceptance producer failed closed: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 1
