...
def finalize(runtime_path: Path, before_path: Path, after_path: Path, absence_path: Path, output_path: Path) -> None:
    ...
    producer_snapshot = {name: evidence.get(name) for name in RUNTIME_PRODUCER_GATES}
    task_absent = (
        ...
        and absence.get("container_remaining_count") == 0
        and absence.get("network_remaining_count") == 0
        and absence.get("volume_remaining_count") == 0
        and absence.get("image_tag_remaining_count") == 0
        and absence.get("image_id_remaining_count") == 0
    )
    ...
    if any(evidence.get(name) != producer_snapshot[name] for name in RUNTIME_PRODUCER_GATES):
        raise SystemExit("RF12 finalizer altered producer-owned evidence")
