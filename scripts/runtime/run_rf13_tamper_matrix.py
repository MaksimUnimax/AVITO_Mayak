...
def _run(root: Path, evidence: Path, sha: str) -> int:
    proc = subprocess.run(
        (sys.executable, str(root / "scripts/runtime/verify_rf13_acceptance.py"), str(root), str(evidence), sha),
...
def main(root: Path, evidence: Path, sha: str, output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="rf13-tamper-") as directory:
        ...
        mutate("idempotency-effects", ("idempotency_concurrency_witness", "business_effect_count"), 2)
        mutate("idempotency-terminals", ("idempotency_concurrency_witness", "terminal_record_count"), 2)
        ...
        mutate("history-restorable", ("lifecycle_witness", "restore_after_permanent_delete"), "ACCEPTED")
        mutate("schema-causation", ("physical_schema", "constraints", "ck_beacon_lifecycle_events_actor_causation_pair", "definition"), "")
