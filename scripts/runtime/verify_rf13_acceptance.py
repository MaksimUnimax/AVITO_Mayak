...
def _check_ownership(item: dict[str, Any]) -> None:
    for p in _path(item, "ownership").values():
...
def _check_lifecycle(item: dict[str, Any]) -> None:
    ...
    _require(p["source_url_before_archive"] == p["source_url_after_restore"] and p["revision_id_before_archive"] == p["revision_id_after_restore"] and p["permanent_delete_state"] == "PERMANENTLY_DELETED" and p["rejected_restore"]["exception_class"] == "BeaconRuntimeError", "lifecycle history")
...
def _check_freeze(item: dict[str, Any]) -> None:
    ...
    _require(p["event"]["actor_account_id"] is None and p["event"]["to_state"] == "FROZEN" and p["freeze_event_count"] == 1 and not p["auto_free_observations"], "freeze event")
...
for case, section in ((c, "preparation") for c in REQUIRED_TAMPER_CASES[12:16]): _CASE_PATHS[case] = (f"{section}.{case.removeprefix('preparation_')}", "preparation")
for case, path in {
