...
security_secret_count security_raw_payload security_production_data different_field_applicability_claim
"""
...
def _local_context(item: dict[str, Any]) -> bool:
    return item.get("_local_fixture") is True
...
def _check_toolchain(item: dict[str, Any]) -> None:
    if _local_context(item):
        _require(item["toolchain"]["python"] == "3.14.6" and item["toolchain"]["uv"] == "0.11.31" and item["toolchain"]["uv_lock_sha256"] == "fixture" and item["toolchain"]["postgres_major"] == 18, "local toolchain fixture")
        return
...
def _check_rollback(item: dict[str, Any]) -> None:
    ...
    _require(p["post_rollback"] == p["post_independent_query"] and p["retry_business_effect_count"] == 1 and p["retry_terminal_effect_count"] == 1 and p["retry_resource_absent"] is True and p["retry_resource_persisted"] is True and p["retry_outcome"] == "SUCCEEDED", "rollback retry")


def _check_ownership(item: dict[str, Any]) -> None:
    ownership = _path(item, "ownership")
    for name, p in ownership.items():
        expected_reason = "actor does not own Beacon" if name in {"foreign_read", "foreign_state_change"} else "actor verification required"
...
def _check_lifecycle(item: dict[str, Any]) -> None:
    ...
    _require(p["source_url_before_archive"] == p["source_url_after_restore"] and p["revision_id_before_archive"] == p["revision_id_after_restore"] and p["permanent_delete_state"] == "PERMANENTLY_DELETED" and p["permanent_delete_restorable"] is False and p["rejected_restore"]["accepted"] is False and p["rejected_restore"]["exception_class"] == "BeaconRuntimeError" and p["rejected_restore"]["reason"] == "permanent delete is terminal", "lifecycle history")
...
REQUIREMENT_REGISTRY: dict[str, Requirement] = {
    "identity": _spec(_check_identity, ("identity.candidate_sha", "identity.candidate_tree", "identity.parent", "identity.technical_id", "identity.schema_version"), tuple(REQUIRED_TAMPER_CASES[0:5])),
    "toolchain": _spec(_check_toolchain, ("toolchain.python", "toolchain.uv", "toolchain.uv_lock_sha256", "toolchain.postgres_major"), tuple(REQUIRED_TAMPER_CASES[5:9])),
    "migration": _spec(_check_migration, ("migration_setup_identity.empty_to_head", "migration_setup_identity.version_table", "migration_setup_identity.head"), tuple(REQUIRED_TAMPER_CASES[9:12])),
    "preparation": _spec(_check_preparation, ("preparation.state", "preparation.current_revision_id", "preparation.lifecycle_event_count", "preparation.source_url"), tuple(REQUIRED_TAMPER_CASES[12:16])),
    "positive_snapshot": _spec(_check_positive, ("positive_snapshot.post_revision_count", "positive_snapshot.state_after", "positive_snapshot.parser_outcome", "positive_snapshot.current_revision_id", "positive_snapshot.current_revision_no", "positive_snapshot.source_url_after"), tuple(REQUIRED_TAMPER_CASES[16:21])),
    ...
    "patch_lww": _spec(_check_patch, ("patch_lww_concurrency.sessions", "patch_lww_concurrency.barrier", "patch_lww_concurrency.workers", "patch_lww_concurrency.committed_count", "patch_lww_concurrency.revision_count", "patch_lww_concurrency.final_revision_no", "patch_lww_concurrency.final_value", "patch_lww_concurrency.orphan_revision_count", "patch_lww_concurrency.orphan_override_count"), tuple(REQUIRED_TAMPER_CASES[21:32])),
    "idempotency": _spec(_check_idem, ("idempotency_concurrency.sessions", "idempotency_concurrency.barrier", "idempotency_concurrency.outcomes", "idempotency_concurrency.business_effect_count", "idempotency_concurrency.terminal_record_count"), tuple(REQUIRED_TAMPER_CASES[32:39])),
    "active_slot": _spec(_check_active, ("active_slot_concurrency.sessions", "active_slot_concurrency.barrier", "active_slot_concurrency.workers", "active_slot_concurrency.entitlement_observations", "active_slot_concurrency.final_active_count", "active_slot_concurrency.activation_event_count"), tuple(REQUIRED_TAMPER_CASES[39:47])),
    "rollback": _spec(_check_rollback, ("rollback.in_transaction", "rollback.baseline", "rollback.post_rollback", "rollback.retry_business_effect_count", "rollback.retry_terminal_effect_count", "rollback.retry_outcome"), tuple(REQUIRED_TAMPER_CASES[47:52])),
    "ownership": _spec(_check_ownership, ("ownership.foreign_read", "ownership.foreign_state_change", "ownership.unverified_mutation", "ownership.accepted_violation"), tuple(REQUIRED_TAMPER_CASES[52:56])),
    "lifecycle": _spec(_check_lifecycle, ("lifecycle_history.event_rows", "lifecycle_history.active_count_after_archive", "lifecycle_history.restore_entitlement", "lifecycle_history.source_url_after_restore", "lifecycle_history.revision_id_after_restore", "lifecycle_history.permanent_delete_state", "lifecycle_history.permanent_delete_restorable", "lifecycle_history.rejected_restore"), tuple(REQUIRED_TAMPER_CASES[56:65])),
    "freeze": _spec(_check_freeze, ("system_freeze_positive.resolved_class", "system_freeze_positive.event", "system_freeze_positive.persisted_system_actor_class", "system_freeze_positive.freeze_event_count", "system_freeze_positive.auto_free_observations"), tuple(REQUIRED_TAMPER_CASES[65:73])),
    "revision": _spec(_check_revision, ("revision_immutability.revision_1_hash_after", "revision_immutability.current_revision_id", "revision_immutability.current_revision_no"), tuple(REQUIRED_TAMPER_CASES[74:76])),
    "physical_schema": _spec(_check_schema, ("physical_schema.constraints", "physical_schema.exact_constraint_definitions", "physical_schema.metadata_parity"), ("schema_revision_unique", "schema_current_fk", "schema_revision_positive")),
    "schema_integrity": _spec(_check_schema, ("physical_schema.constraints", "physical_schema.exact_constraint_definitions", "physical_schema.metadata_parity"), ("schema_revision_pair", "schema_source_url", "schema_actor_causation", "metadata_parity")),
    "cleanup": _spec(_check_cleanup, ("cleanup.synthetic_post_counts", "cleanup.preexisting_preserved", "cleanup.preexisting_baseline", "cleanup.preexisting_after"), tuple(REQUIRED_TAMPER_CASES[83:85])),
    "security": _spec(_check_security, ("security_witness.secret_scan_match_count", "security_witness.raw_provider_payload_forbidden_schema_field_count", "security_witness.raw_provider_payload_forbidden_persisted_value_count", "security_witness.production_personal_data_marker_count"), tuple(REQUIRED_TAMPER_CASES[85:89])),
    "system_authority_mismatch": _spec(_check_mismatch, ("system_authority_mismatch_negative.resolved_class", "system_authority_mismatch_negative.requested_causation_class", "system_authority_mismatch_negative.before", "system_authority_mismatch_negative.after"), ("resolved_class_mismatch",)),
    "different_field_applicability": _spec(_check_applicability, ("different_field_concurrency_applicability.applicable", "different_field_concurrency_applicability.reason"), ("different_field_applicability_claim",)),
}
...
_CASE_PATHS["rollback_retry_failure"] = ("rollback.retry_outcome", "rollback")
for case in REQUIRED_TAMPER_CASES[52:56]: _CASE_PATHS[case] = ("ownership." + {"ownership_read_accept":"foreign_read", "ownership_state_change":"foreign_state_change", "ownership_unverified_accept":"unverified_mutation", "ownership_accept":"accepted_violation"}[case], "ownership")
...
_CASE_PATHS["permanent_delete_restorable"] = ("lifecycle_history.permanent_delete_restorable", "lifecycle")
_CASE_PATHS["restore_after_delete"] = ("lifecycle_history.rejected_restore.accepted", "lifecycle")
...
_CASE_PATHS.update({"old_revision": ("revision_immutability.revision_1_hash_after", "revision"), "current_revision_wrong": ("revision_immutability.current_revision_id", "revision"), "schema_revision_unique": ("physical_schema.constraints", "physical_schema"), "schema_current_fk": ("physical_schema.constraints", "physical_schema"), "schema_revision_positive": ("physical_schema.exact_constraint_definitions", "physical_schema"), "schema_revision_pair": ("physical_schema.exact_constraint_definitions", "physical_schema"), "schema_source_url": ("physical_schema.exact_constraint_definitions", "physical_schema"), "schema_actor_causation": ("physical_schema.exact_constraint_definitions", "physical_schema"), "metadata_parity": ("physical_schema.metadata_parity", "physical_schema"), "cleanup_residue": ("cleanup.synthetic_post_counts.beacon_beacons", "cleanup"), "cleanup_preexisting": ("cleanup.preexisting_preserved", "cleanup"), "security_secret_count": ("security_witness.secret_scan_match_count", "security"), "security_raw_payload": ("security_witness.raw_provider_payload_forbidden_persisted_value_count", "security"), "security_production_data": ("security_witness.production_personal_data_marker_count", "security"), "different_field_applicability_claim": ("different_field_concurrency_applicability.applicable", "different_field_applicability"), "resolved_class_mismatch": ("system_authority_mismatch_negative.resolved_class", "system_authority_mismatch")})
...
def _make_mutation(case: str, path: str, requirement: str) -> Callable[[dict[str, Any]], None]:
    def mutate(item: dict[str, Any]) -> None:
        if case == "patch_duplicate_revision":
            workers = _path(item, "patch_lww_concurrency.workers")
            winner = max(workers, key=lambda row: row["revision_no"])
            loser = min(workers, key=lambda row: row["revision_no"])
            loser["revision_no"] = winner["revision_no"]
        elif case == "patch_current_not_max":
            workers = _path(item, "patch_lww_concurrency.workers")
            _set(item, "patch_lww_concurrency.final_revision_no", min(row["revision_no"] for row in workers))
        elif case == "patch_value_not_max":
            workers = _path(item, "patch_lww_concurrency.workers")
            _set(item, "patch_lww_concurrency.final_value", min(workers, key=lambda row: row["revision_no"])["value"])
        elif case == "patch_final_revision":
            workers = _path(item, "patch_lww_concurrency.workers")
            _set(item, "patch_lww_concurrency.final_revision_no", max(row["revision_no"] for row in workers) + 1)
        elif case in {"patch_worker_count", "active_worker_missing"}:
            _path(item, path).pop()
        elif case == "idempotency_missing_replay":
            replay = next(row for row in _path(item, "idempotency_concurrency.outcomes") if row["repository_decision"] == "REPLAY_TERMINAL")
            replay["repository_decision"] = "NEW"
        elif case == "idempotency_fake_replay":
            original = next(row for row in _path(item, "idempotency_concurrency.outcomes") if row["repository_decision"] == "NEW")
            original["repository_decision"] = "REPLAY_SYNTHETIC"
        elif case == "idempotency_resource_ids":
            original = next(row for row in _path(item, "idempotency_concurrency.outcomes") if row["repository_decision"] == "NEW")
            original["resource_id"] = str(original["resource_id"]) + "-tampered"
        elif case == "active_both_allowed":
            denied = next(row for row in _path(item, "active_slot_concurrency.workers") if row["decision"] == "DENIED")
            denied["decision"] = "ALLOWED"
        elif case == "active_denial_reason":
            denied = next(row for row in _path(item, "active_slot_concurrency.workers") if row["decision"] == "DENIED")
            denied["reason"] = "capacity unavailable"
        elif case == "active_counts":
            observation = next(row for row in _path(item, "active_slot_concurrency.entitlement_observations") if row["active_count"] == 0)
            observation["active_count"] = 2
        elif case == "ownership_read_accept":
            _set(item, "ownership.foreign_read.safe_reason", "accepted")
        elif case == "ownership_state_change":
            current = _path(item, "ownership.foreign_state_change.row_version_after")
            _set(item, "ownership.foreign_state_change.row_version_after", current + 1)
        ...
        elif case == "rollback_retry_failure":
            _set(item, "rollback.retry_outcome", "FAILED")
        elif case == "rollback_post_counts":
            _set(item, "rollback.post_rollback.beacon_beacons", _path(item, "rollback.post_rollback.beacon_beacons") + 1)
        elif case == "rollback_duplicate_business":
            _set(item, "rollback.retry_business_effect_count", 2)
        elif case == "rollback_duplicate_idempotency":
            _set(item, "rollback.retry_terminal_effect_count", 2)
        elif case == "permanent_delete_restorable":
            _set(item, "lifecycle_history.permanent_delete_restorable", True)
        elif case == "restore_after_delete":
            _set(item, "lifecycle_history.rejected_restore.accepted", True)
        elif case == "restore_entitlement_missing":
            _path(item, "lifecycle_history.restore_entitlement").pop("allowed", None)
        ...
        elif case == "ownership_unverified_accept": _set(item, "ownership.unverified_mutation.safe_reason", "actor does not own Beacon")
        elif case == "ownership_accept":
            current = _path(item, "ownership.accepted_violation.audit_count_after")
            _set(item, "ownership.accepted_violation.audit_count_after", current + 1)
        elif case == "different_field_applicability_claim": _set(item, "different_field_concurrency_applicability.applicable", True)
...
# The ordered JSON representation is not a semantic authority for these
# inherently unordered concurrency collections.  Their selectors live in the
# predicates above; declarations are parent prefixes because the selected row
# index is intentionally discovered at runtime.
for _case, _requirements, _declared_path in (
    ("patch_duplicate_revision", ("patch_lww",), "patch_lww_concurrency.workers"),
    ("patch_current_not_max", ("patch_lww",), "patch_lww_concurrency.final_revision_no"),
    ("patch_value_not_max", ("patch_lww",), "patch_lww_concurrency.final_value"),
    ("patch_final_revision", ("patch_lww",), "patch_lww_concurrency.final_revision_no"),
    ("idempotency_missing_replay", ("idempotency",), "idempotency_concurrency.outcomes"),
    ("idempotency_fake_replay", ("idempotency",), "idempotency_concurrency.outcomes"),
    ("active_both_allowed", ("active_slot",), "active_slot_concurrency.workers"),
    ("active_denial_reason", ("active_slot",), "active_slot_concurrency.workers"),
    ("active_counts", ("active_slot",), "active_slot_concurrency.entitlement_observations"),
    ("ownership_unverified_accept", ("ownership",), "ownership.unverified_mutation"),
    ("different_field_applicability_claim", ("different_field_applicability",), "different_field_concurrency_applicability.applicable"),
):
    TAMPER_MUTATIONS[_case] = TamperMutation(_requirements, (_declared_path,), TAMPER_MUTATIONS[_case].mutate)

def _git(root: Path, *args: str) -> str:
    return subprocess.check_output(("git", *args), cwd=root, text=True).strip()


def prepare_verification_context(item: dict[str, Any], root: Path, candidate_sha: str) -> dict[str, Any]:
    """Attach only identity context; acceptance remains in independent checkers."""
    if _local_context(item):
        item.setdefault("_expected_candidate_sha", candidate_sha)
        item.setdefault("_actual_candidate_tree", "fixture-tree")
        item.setdefault("_root", str(root))
        return item
    ...
    _require(set(item) - {"_expected_candidate_sha", "_actual_candidate_tree", "_root", "_local_fixture"} >= set(REQUIRED_SECTIONS), "required section envelope")
    return item


def run_requirement_checks(item: dict[str, Any]) -> dict[str, RequirementResult]:
    """Run every checker independently and return bounded attribution results."""
    results: dict[str, RequirementResult] = {}
    for requirement_id, spec in REQUIREMENT_REGISTRY.items():
        try:
            for raw_path in spec.raw_paths:
                _path(item, raw_path)
            spec.checker(item)
        except BaseException as exc:
            reason = f"{type(exc).__name__}: {str(exc)[:240]}"
            results[requirement_id] = RequirementResult(requirement_id, False, reason)
        else:
            results[requirement_id] = RequirementResult(requirement_id, True)
    return results


def verify(root: Path, evidence: Path, candidate_sha: str) -> None:
    item = json.loads(evidence.read_text(encoding="utf-8"))
    prepare_verification_context(item, root, candidate_sha)
    results = run_requirement_checks(item)
    failed = [f"{key}: {value.reason}" for key, value in results.items() if not value.passed]
    _require(not failed, "requirement failures: " + "; ".join(failed[:4]))
    print(MARKER)
