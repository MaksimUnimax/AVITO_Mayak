...
def verify(root: Path, evidence: Path, candidate_sha: str) -> None:
    ...
    _require(
        all(constraints[name].get("definition") for name in required_constraints),
        "constraint definition missing",
    )
    ...
    _require(
        sorted(row.get("decision") for row in active["workers"]) == ["DENIED", "ALLOWED"],
        "active-slot outcomes",
    )
    _require(
