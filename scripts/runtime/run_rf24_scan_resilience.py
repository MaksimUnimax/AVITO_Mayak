...
def produce(root: Path, output: Path, probes: Path, log: Path, source_sha: str) -> None:
    ...
    try:
        ...
        _stop(scheduler_2)
        scheduler_after_first = _owning_snapshot(composition, account_id=account, beacon_id=beacon_id, work_id=str(sched_first["work_item_id"]))
        evaluation_count = len([row for row in _records(scheduler_obs, run_id) if row.get("record_type") == "scheduler_evaluation"])
        scheduler_3 = _proc("mayak.runtime.scheduler", {**base_env, "MAYAK_PROCESS_KIND": "mayak-scheduler", "RF24_PROCESS_GENERATION": "S3"}, workdir / "rf24-scheduler.log")
        processes.append(scheduler_3)
        _wait_after(scheduler_obs, run_id, "mayak-scheduler", "scheduler_evaluation", evaluation_count)
        _stop(scheduler_3)
        scheduler_after = _owning_snapshot(composition, account_id=account, beacon_id=beacon_id, work_id=str(sched_first["work_item_id"]))
        scheduler_item = {"scenario_name": "scheduler-restart", "acceptance_run_id": run_id, "source_sha": actual, "account_id": account, "beacon_id": beacon_id, "schedule_id": schedule_id, "work_id": sched_first["work_item_id"], "before": {"persistent_schedule": True, "materialized_work_count": 0}, "action": {"public_boundary": ACTION_BOUNDARIES["scheduler-restart"], "actual_action_invoked": "S1 terminated before due; S2 materialized; S3 re-evaluated durable schedule", "observation_source": "scheduler JSONL process observation", "process_observed": True, "pid_1": scheduler_1.pid, "pid_2": scheduler_3.pid, "materializer_pid": sched_first["process_pid"], "generation_1": "S1", "generation_2": "S2", "generation_3": "S3"}, "after": {"persistent_schedule": True, "materialized_work_count": 1}, "durable_before": {**scheduler_before, "schedule_exists": True, "work_count": 0}, "durable_after": {**scheduler_after, "schedule_exists": True, "work_count": 1}, "durable_after_first_materialization": {**scheduler_after_first, "schedule_exists": True, "work_count": 1}, "notification_deltas": {"source_intake": 0, "outbox_effect": 0, "delivery_attempt": 0, "observation_source": "notification-delivery-owned-read"}}
