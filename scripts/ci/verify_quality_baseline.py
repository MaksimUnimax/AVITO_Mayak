...
def validate_suite_observations(
    ...
    if minimum_count is not None and collection_run_1_count < minimum_count:
        raise RuntimeError("test count below explicit comparison-base collection")
    if execution["executed_collected_count"] != collection_run_1_count:
        raise RuntimeError("execution count differs from collection")
    if execution["failed_count"] or execution["error_count"]:
        raise RuntimeError("execution outcome mismatch")
    if execution["xpassed_count"] or execution["xfailed_count"]:
        raise RuntimeError("unexpected xfail outcome")
    if (
        execution["passed_count"] + execution["skipped_count"]
        != execution["executed_collected_count"]
    ):
        raise RuntimeError("passed and governed skipped counts do not account for execution")
...
def _ruff_observation(exe: Path, cwd: Path, evidence: Path, label: str) -> dict[str, object]:
    ...
    record(
        ...
        f"ruff-{label}",
        cp,
        {"diagnostic_count": len(identities), "normalized_sha256": digest(normalized)},
    )
    return {
...
def _materialize_ref(ref: str, destination: Path) -> None:
    ...
    cp = subprocess.run(
        ["git", "archive", "--format=tar", ref, "-o", str(archive)],
        cwd=ROOT,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
...
def mypy_gate(evidence: Path, venv: Path, base_sha: str | None = None) -> dict:
    ...
    return {
        ...
        "classification": "MYPY_EXPLICIT_BASE_NO_REGRESSION"
        if base_sha
        else "MYPY_PREEXISTING_DEBT_NO_REGRESSION",
    }
...
def static_quality_gate(evidence: Path, venv: Path, base_sha: str) -> dict:
    ...
    return {
        ...
        "pytest": {"authority": "canonical-db-backed-reusable-workflow"},
        "coverage": {"authority": "scripts/ci/verify_coverage_baseline.py"},
    }
...
def main() -> int:
    ...
    try:
        result = (
            lock_gate(evidence)
            if args.mode == "lock"
            else static_quality_gate(evidence, args.venv.resolve(), args.base_sha or "")
        )
        payload = {
            "schema_version": 1,
