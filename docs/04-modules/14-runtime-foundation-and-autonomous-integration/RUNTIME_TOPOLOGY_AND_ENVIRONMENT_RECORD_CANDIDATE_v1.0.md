...
The private backend network is the project-owned logical key `mayak-internal`; the API additionally uses the project-owned `mayak-loopback` bridge solely for deterministic host-local ingress. PostgreSQL, worker, scheduler and one-shot services remain only on `mayak-internal`; only the API joins both networks. The loopback bridge is Compose-managed, standard (not `internal`), uses `com.docker.network.bridge.host_binding_ipv4=127.0.0.1`, and is not public ingress. The API remains published only as `127.0.0.1:<selected-18080-18099>:8000`; PostgreSQL has no host port. Logical named volume key remains `postgres-data`. Compose-generated resource names remain project-scoped. Optional services are disabled by profile/default unless an exact future task enables them; live provider service/profile is not enabled by default.
...
| `mayak-api` | mandatory process | one project application image | FastAPI/Uvicorn API | `mayak-internal` + `mayak-loopback` (loopback has higher gateway priority) | none | application core references | future `127.0.0.1:<selected-18080-18099>:8000` | migration success and DB ready | unless-stopped | RF-23 | RF-24/RF-26 |
...
The API host bind is exactly `127.0.0.1`; candidate host range is exactly `18080–18099`. There is no public bind: no `0.0.0.0`, no `[::]`, and no public interface. PostgreSQL is never published to the host. Operator browser access is through localhost or an SSH tunnel. Only the two project-owned Compose bridges are used: `mayak-internal` is the private backend and PostgreSQL service-discovery path; `mayak-loopback` is an API-only host-local ingress boundary, not a shared frontend. There is no foreign or external Docker network and no automatic connection to a foreign proxy/network. Provider outbound is disabled by configuration by default. No Nginx, firewall, DNS or TLS dependency is introduced.
