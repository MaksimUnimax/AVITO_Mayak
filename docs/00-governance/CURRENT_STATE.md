...
## RF-08 observed cleanup and foreign recomputation — 2026-07-30

The e547 candidate remained published but independently unaccepted. The first continuation attempt was not published because its toolchain and Docker identity source were incomplete. The final candidate uses exact Python 3.14.6 and uv 0.11.31 with frozen development dependencies; missing optional Docker `.ID` is not authority. Local Docker endpoint instance identity is derived from actual Unix socket, peer, process and server-version observations. Stages 56–57 derive fields from typed observations and mutation records, and the verifier independently reconstructs the verdict. Docker-native image-input evidence remains unchanged. RF-11 remains preserved and unaccepted; RF-12 and RF-23 remain unstarted. Status is `PUBLISHED_FOR_CHATGPT_REVIEW`; `NOT_PRODUCTION_READY` remains authoritative.

## RF-08 pre-execution Docker authority corrective — 2026-07-30

Published main `7467ba2092d9e83fba52d402cf78556912d10a83` remained published but was independently rejected because its mutation ledger was post-execution, producer/verifier classifiers diverged, endpoint fallback was mislabeled, preflight claims were hardcoded, tests were insufficient and foreign topology evidence was excessive. This corrective authorizes and records every Docker mutation before execution, independently reconstructs exact ownership and delta semantics, recursively validates minimized evidence, and proves the complete 57-stage protocol. RF-11 remains preserved and unaccepted; RF-12 and RF-23 remain unstarted. Status is `PUBLISHED_FOR_CHATGPT_REVIEW`; `NOT_PRODUCTION_READY` remains authoritative.
