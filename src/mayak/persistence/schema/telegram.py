"""Module 09 Telegram Adapter physical schema registration."""
...
_NAMES = ("telegram_inbound_updates", "telegram_identity_mappings", "telegram_delivery_mappings")
...
def _canonical(metadata: MetaData) -> tuple[Table, Table, Table]:
    inbound = Table(
        ...
        Column("provider_update_id", String(255), nullable=False),
        ...
        CheckConstraint(
            "btrim(provider_update_id) <> ''", name=conv("provider_update_id_nonempty")
        ),
        CheckConstraint(
    ...
    Index("ix_telegram_inbound_updates_provider_update_id", inbound.c.provider_update_id)
    Index("ix_telegram_inbound_updates_received_at", inbound.c.received_at)
    ...
    identity = Table(
        ...
        Column("telegram_user_ref", String(255), nullable=False),
        ...
        UniqueConstraint("provider_link_id", name="uq_telegram_identity_mappings_provider_link_id"),
        CheckConstraint("btrim(telegram_user_ref) <> ''", name=conv("telegram_user_ref_nonempty")),
        CheckConstraint("row_version > 0", name=conv("row_version_positive")),
    )
    Index("ix_telegram_identity_mappings_provider_link_id", identity.c.provider_link_id)
    ...
    delivery = Table(
        ...
        Column("telegram_message_ref", String(255), nullable=True),
        Column("created_at", TIMESTAMP(timezone=True), nullable=False),
        ForeignKeyConstraint(
            ["attempt_id"], ["mayak.notification_delivery_attempts.id"], ondelete="RESTRICT"
        ),
        UniqueConstraint("attempt_id", name="uq_telegram_delivery_mappings_attempt_id"),
        CheckConstraint(
            "telegram_message_ref IS NULL OR btrim(telegram_message_ref) <> ''",
            name=conv("telegram_message_ref_nonempty_when_present"),
        ),
    )
    Index(
        "ux_telegram_delivery_mappings_message_ref",
        delivery.c.telegram_message_ref,
        unique=True,
        postgresql_where=text("telegram_message_ref IS NOT NULL"),
    )
    return inbound, identity, delivery


def register_telegram_tables(target_metadata: MetaData) -> tuple[Table, Table, Table]:
    """Attach or validate the three Telegram Adapter tables without I/O."""
    if target_metadata.schema != "mayak":
        raise RuntimeError("telegram tables require mayak schema")
    if _stable(target_metadata.naming_convention) != _stable(_CONVENTION):
        raise RuntimeError("conflicting existing telegram metadata")
    if _stable(target_metadata.info) != _stable({}):
        raise RuntimeError("conflicting existing telegram metadata")
    missing = tuple(
        _key(target_metadata, n)
        for n in ("identity_provider_links", "notification_delivery_attempts")
        if _key(target_metadata, n) not in target_metadata.tables
    )
    if missing:
        raise RuntimeError("missing telegram prerequisites: " + ", ".join(missing))
    present = tuple(n for n in _NAMES if _key(target_metadata, n) in target_metadata.tables)
    if present and present != _NAMES:
        raise RuntimeError("partial telegram table registration")
    ...
    if existing_order and existing_order != _NAMES:
        raise RuntimeError("conflicting telegram table order")
    ...
    if present == _NAMES:
        ...
        if any(_table_signature(a) != _table_signature(e) for a, e in zip(actual, expected)):
            raise RuntimeError("conflicting existing telegram registration")
        return cast(tuple[Table, Table, Table], actual)
    return _canonical(target_metadata)


__all__ = ["register_telegram_tables"]
