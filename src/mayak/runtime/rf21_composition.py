...
class IdentityWebAdapter:
    ...
    def resolve_session(self, session: Any, session_reference: Any) -> VerifiedWebCustomer | None:
        ...
        return VerifiedWebCustomer(
            result.account_id, result.metadata.session_id,
...
class CustomerIdentityAuthorityAdapter:
    ...
    def resolve(self, session: Any, *, actor_reference: Any,
                requested_account_id: UUID | None) -> ResolvedActor:
        validation = self.runtime.validate_session(session, actor_reference)
        ...
        return ResolvedActor(
            actor_id=validation.account_id, account_id=validation.account_id,
            verified=True, reference=f"identity-session:{validation.metadata.session_id}",
        )
...
class ScanWebAdapter:
    ...
    def read(self, session: Any, customer: VerifiedWebCustomer) -> dict[str, Any]:
        self.calls += 1
        return {"owner": self.owner, "beacons": [
...
def build_rf21_runtime(*, identity: IdentityRuntime, beacon: BeaconManagementRuntime,
                       ...
                       max_adapter: Any | None = None, support: Any | None = None,
                       settings: Any | None = None) -> Any:
    ...
    projections: list[Any] = [
        EntitlementWebAdapter(entitlements), NotificationWebAdapter(notification),
        scan or ScanWebAdapter(beacon_adapter),
    ]
    if effective_settings is not None:
        projections.extend((telegram or TelegramWebAdapter(effective_settings),
                            max_adapter or MaxWebAdapter(effective_settings)))
    else:
        projections.extend((telegram or SafeUnavailableAdapter("telegram_adapter", "telegram"),
                            max_adapter or SafeUnavailableAdapter("max_adapter", "max")))
    projections.append(SupportWebAdapter(support) if support is not None
                       and not hasattr(support, "read") else
    ...
    return WebCabinetRuntime(
        ...
        beacon=beacon_adapter, projections=tuple(projections),
    )


__all__ = ["IdentityWebAdapter", "BeaconWebAdapter", "EntitlementWebAdapter",
           "CustomerIdentityAuthorityAdapter", "NotificationWebAdapter", "ScanWebAdapter",
           "TelegramWebAdapter", "MaxWebAdapter",
           "SupportWebAdapter", "SafeUnavailableAdapter", "build_rf21_runtime"]
