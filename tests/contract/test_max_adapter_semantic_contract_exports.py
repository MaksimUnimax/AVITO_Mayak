...
def test_package_exports_are_exact_and_ordered():
    assert package.__all__ == ["MODULE_ID", *CONTRACTS, *RUNTIME_EXPORTS]
...
def test_authoritative_records_and_enums_are_exact():
    ...
    assert all(
        getattr(contracts, n).__module__.endswith("max_adapter.contracts") for n in RECORDS + ENUMS
    )
...
def test_import_has_no_provider_runtime_side_effect():
    ...
    assert not any(n.split(".")[0] in {"requests", "httpx", "aiohttp", "sqlalchemy"} for n in added)
