...
def tg08_architecture_guard(source_override: str | None = None) -> None:
    direct_entries = {path.name for path in PRODUCTION.iterdir()}
    assert direct_entries - ALLOWED <= {"__pycache__"}
...
def test_cache_safe_guard_rejects_unexpected_direct_entry(
    ...
    monkeypatch.setattr(
        "tests.architecture.test_telegram_adapter_deep_link_validation_boundaries.PRODUCTION",
        tmp_path,
    )
    source = (
        Path(__file__).parents[2] / "src/mayak/modules/telegram_adapter/contracts.py"
    ).read_text()
    tg08_architecture_guard(source)
