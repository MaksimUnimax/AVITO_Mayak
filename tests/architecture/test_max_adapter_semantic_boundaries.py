...
FILES = [
    ROOT / "src/mayak/modules/max_adapter/__init__.py",
...
PACKAGE_INIT_IMPORTS = {
    ...
    "mayak.modules.max_adapter.contracts",
    ".mini_app",
    ".runtime",
    ".transport",
}
FORBIDDEN_CLASS_PARTS = (
    "handler",
    "client",
    "gateway",
    "repository",
...
def violations(
    source: str, *, package_init: bool = False, production_file: Path | None = None
) -> list[str]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            for module in _module_name(node):
                normalized = (
                    "mayak.modules.max_adapter.contracts"
                    if package_init and module == ".contracts"
                    else module
                )
        ...
        if isinstance(node, (ast.AnnAssign, ast.Assign)):
            ...
            if any(name.lower() in SENSITIVE_FIELDS for name in names):
                result.extend(
                    f"sensitive-field:{name}" for name in names if name.lower() in SENSITIVE_FIELDS
                )
    ...
    if production_file is not None:
        direct_entries = sorted(
            path.name for path in production_file.parent.iterdir() if path.name != "__pycache__"
        )
        if direct_entries != [
            "__init__.py",
            "contracts.py",
...
def test_production_file_set_ignores_only_pycache(tmp_path):
    ...
    for name in ("mini_app.py", "runtime.py", "transport.py"):
...
def test_no_forbidden_declarations():
    assert all(
        node.name.lower() not in FORBIDDEN_CLASS_NAMES
        and not any(part in node.name.lower() for part in FORBIDDEN_CLASS_PARTS)
        for path in FILES
...
UNSAFE_SNIPPETS = [
    ...
    ("forbidden relative import", "from .unknown import Client", True),
