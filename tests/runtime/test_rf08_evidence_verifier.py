...
def test_recursive_minimization_rejects_nested_sensitive_values() -> None:
    for value in ({"nested": {"dsn": "postgresql://user:pass@db/x"}},
...
def test_valid_minimized_record_and_digest_pass() -> None:
    value = {"source_host_safe_identity": "a" * 64, "object_identity_hash": "b" * 64,
             "ownership": "FOREIGN", "mount_count": 0}
    validate_safe_value(value)
...
def test_incomplete_or_unresolved_snapshot_rejected() -> None:
    snapshot = {
        "schema_version": "ForeignResourceSnapshotV3", "collector_implementation_id": "collector",
        "collection_complete": False, "collection_errors": ["timeout"], "redaction_passed": True,
    }
    with pytest.raises(ValueError):
...
def test_duplicate_identity_rejected() -> None:
    base = {
        "schema_version": "ForeignResourceSnapshotV3", "capture_phase": "before",
        "collector_implementation_id": "collector", "capture_monotonic_sequence": 1,
        "source_host_safe_identity": "a" * 64, "host_boot_instance_safe_identity": "b" * 64,
        "docker_server_safe_identity": "c" * 64, "docker_endpoint_identity_schema": "LOCAL_UNIX_DOCKER_ENDPOINT_SOCKET_V1",
        ...
        "collection_complete": True, "collection_errors": [], "redaction_passed": True,
