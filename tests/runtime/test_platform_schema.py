...
def test_registration_is_idempotent() -> None:
    ...
    assert first == second
    assert len(metadata.tables) == 36
    assert sum(len(table.indexes) for table in metadata.tables.values()) == 51
...
def test_audit_columns_constraints_indexes_and_deferred_marker() -> None:
    ...
    assert [e.target_fullname for e in fks[0].elements] == ["mayak.identity_accounts.id"]
...
def test_outbox_columns_defaults_constraints_and_partial_indexes() -> None:
    ...
    assert (
        "state IN ('PENDING', 'RETRY')" in predicates["ix_platform_event_outbox_available"]  # type: ignore[index]
    )
