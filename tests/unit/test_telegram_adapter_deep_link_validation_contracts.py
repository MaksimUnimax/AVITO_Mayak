...
def _parts(
    *,
    ...
    effective_handoff = handoff_ref if not decision_required or context_decision else None
    evidence = TelegramDeepLinkContextResolutionEvidence(
        ...
        server_side_context_resolution_reference=context_ref,
    ...
    dedup = TelegramUpdateDeduplicationRecord(
        ...
        metadata=_meta(),
        provider_update_identity=update,
        idempotency_key=IdempotencyKey(value="dedup-key"),
        idempotency_scope=IdempotencyScope(value="telegram-scope"),
        fingerprint=IdempotencyFingerprint(value="dedup-fp"),
        state=state,
        ...
        if state is TelegramUpdateDeduplicationState.NEW_UPDATE
        else IdempotencyFingerprint(value="dedup-fp"),
...
def _outcome(
    ...
    return TelegramDeepLinkValidationOutcome(
        ...
        owner_handoff_reference="handoff-ref"
        if state
        in {
