...
def test_boundary_guard_rejects_unfiltered_workflow_and_script() -> None:
    ...
    assert not module._has_unfiltered_pytest(
        "uv run pytest -q tests/ci/test_x.py 2>&1 | tee focused.log"
    )
    assert not module._has_unfiltered_pytest("uv run pytest -q\n  tests/ci/test_x.py")
    assert (
        module.validate_regression_boundaries(root).get(".github/workflows/ci-rf26-operability.yml")
        is None
    )
...
def test_boundary_guard_adversarial_relationships(tmp_path: Path) -> None:
    ...
    (workflow_dir / "ci-full-regression.yml").write_text(
        "coverage run -m pytest\n", encoding="utf-8"
    )
    (workflow_dir / "ci-rf-test.yml").write_text(
        "uv run pytest -q 2>&1 | tee all.log\ngit config --global x y\n",
        encoding="utf-8",
    )
